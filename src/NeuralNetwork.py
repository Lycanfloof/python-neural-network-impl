...
class NeuralNetwork:
  def __init__(self, cost_function, cost_prime, layers = [], learning_rate = 0.1) -> None:
  ...
  def fit(self, x_train, y_train, epochs = 50, verbose = False) -> None:
    for epoch in range(epochs):
      ...
      self.__backward_propagation(cost)
  ...
  def __backward_propagation(self, cost) -> None:
    current_cost = cost

    for layer in reversed(self.layers):
      current_cost = layer.backward_propagation(current_cost, self.learning_rate)
